...
Table~\ref{tab:respin_comprehensive} presents the official test evaluation matrix evaluated across 2,170 utterances on the \texttt{IISc\_RESPIN\_test\_mr} test set, comparing non-finetuned zero-shot base models, fine-tuned IndicBART (16k original vs.\ 32k expanded), and fine-tuned google/mT5-small (16k original vs.\ 32k expanded).
...
\hline
\textit{Non-Finetuned Base} & Zero-Shot & 2,170 & 3.12 & 704.22\% & 2.45 & $>500.00\%$ \\
\hline
...
\begin{enumerate}
    \item \textbf{Failure of Zero-Shot Base Models}: Non-finetuned base models (\texttt{IndicBART} and \texttt{mT5-small}) fail completely in zero-shot mode, exhibiting severe repetitive token hallucination and producing degenerate outputs ($>500\%$ WER, BLEU $< 3.5$). Supervised fine-tuning is strictly required for task alignment.
...
\subsection{\textcolor{black}{Inference Latency and Decoding Search Ablation}}

To evaluate real-time deployment viability, Table~\ref{tab:latency_beam} compares inference latency (milliseconds per sentence) and throughput (tokens per second) across decoding search strategies (Greedy vs.\ Beam Search $k=3, 5$) on both GPU (NVIDIA T4, batch size 16) and CPU (Intel Core i7-1185G7 @ 3.0GHz, single thread) hardware environments.

\begin{table}[t]
\color{black}
\centering
\caption{\textcolor{black}{Inference Latency and Decoding Search Strategy Ablation (mT5-Small 32k)}}
\label{tab:latency_beam}
\begin{tabularx}{\textwidth}{llcccc}
\hline
\textbf{Decoding Mode} & \textbf{Hardware} & \textbf{Latency (ms/sent)} & \textbf{Throughput (tok/s)} & \textbf{BLEU} & \textbf{chrF++} \\
\hline
Greedy ($k=1$) & GPU (NVIDIA T4) & \textbf{14.2 ms} & \textbf{412 tok/s} & 72.85 & 87.10 \\
Beam Search ($k=3$) & GPU (NVIDIA T4) & 28.5 ms & 210 tok/s & \textbf{73.48} & \textbf{87.70} \\
Beam Search ($k=5$) & GPU (NVIDIA T4) & 42.1 ms & 142 tok/s & 73.52 & 87.74 \\
\hline
Greedy ($k=1$) & CPU (Intel i7) & \textbf{84.6 ms} & \textbf{68 tok/s} & 72.85 & 87.10 \\
Beam Search ($k=3$) & CPU (Intel i7) & 192.4 ms & 30 tok/s & 73.48 & 87.70 \\
\hline
\end{tabularx}
\end{table}

Greedy decoding ($k=1$) achieves a \textbf{2.0$\times$ speedup} on GPU (14.2 ms/sentence) with negligible BLEU drop ($-$0.63 BLEU vs.\ Beam 3), establishing an optimal low-latency configuration for real-time mobile voice assistant integration.
