...
The task of normalizing dialectal text into a standardized written form has been formally established as a distinct sentence-level character transduction task alongside historical and user-generated content normalization \cite{kuparinen2023dialect}. Early neural explorations by Partanen et al. \cite{partanen2019dialect} demonstrated that sliding-window chunk-level neural networks effectively normalize 23 Finnish dialect varieties, reducing initial word error rates from 52.89\% to 5.73\%. Kuparinen et al. \cite{kuparinen2023dialect} subsequently compiled a large-scale multilingual benchmark across Finnish, Norwegian, Swiss German, and Slovene, demonstrating that sequence-to-sequence transformers and pre-trained byte-level models (byT5) consistently outperform statistical and recurrent baselines.
...
In the Germanic language family, Swiss German dialect normalization to Standard High German has been studied extensively due to the absence of a standardized written form for Swiss German dialects. Early researchers explored rule-based and statistical character-level models, while recent work by Kresic and Abbas \cite{kresic2024normalizing} demonstrated that fine-tuning multilingual sequence-to-sequence transformers on the SwissDial dataset achieves high-quality text normalization, with the compact mT5-small variant performing comparably to base and large model sizes.

For Indic languages, dialect normalization remains a significantly underexplored area. While some work has addressed code-mixing (e.g., Hindi-English) and transliteration, the specific task of mapping sub-regional dialectal text to a standardized written form within the same language has received minimal computational attention, particularly for Marathi and its diverse regional varieties. A comprehensive survey by Joshi et al. \cite{joshi2025survey} categorizes dialectal NLP tasks across Natural Language Understanding (NLU) and Natural Language Generation (NLG) across diverse language families, emphasizing the performance degradation of state-of-the-art models on non-standard dialects and underscoring the urgent need for equitable language technologies. Recently, Dimakis et al. \cite{dimakis2025dialect} introduced a hybrid normalization framework combining rule-based morphological transformations with LLM few-shot prompting to standardize low-resource Greek dialect proverbs without requiring prior parallel data. Concurrently, Mulla and Kumar \cite{mulla2022text} explored spectro-temporal feature extraction for text-independent Marathi dialect recognition across LDC-IL audio corpora, Amartyaveer et al. \cite{amartyaveer2025improving} developed multimodal ASR and RoBERTa embeddings for dialect identification across Indian languages (including Marathi) under the RESPIN initiative, Gaikwad et al. \cite{gaikwad2025marathi} explored Retrieval-Augmented Generation (RAG) prompting via the Gemini API for zero-shot Marathi dialect translation, Pawar et al. \cite{pawar2026automatic} investigated acoustic dialect classification (Ahirani, Varhadi, Standard Marathi) using MFCC feature extraction and CNNs in noisy environments, Vyavhare et al. \cite{vyavhare2026marathi} proposed a hybrid rule-based and neural translation framework for regional Marathi dialects, and Gavali \cite{gavali2026marathi} introduced a student-level framework combining IndicBERT and IndicWav2Vec for dialect detection and normalization on small local datasets. Our work significantly advances this landscape by establishing a large-scale 32,335-pair parallel benchmark, introducing an automated Gemma-2 LLM synthetic data expansion pipeline, and evaluating pre-trained sequence-to-sequence transformer architectures (mT5-small, IndicBART, IndicTrans2) across multiple dataset configurations.
...
\textbf{Tier 1: Rule-Based Syntactic Filter.} Automated validation checks implement Indic Abugida script normalization principles \cite{ansary2024unicode}, including: (a) Devanagari script integrity---both source and target must contain $>$90\% Devanagari Unicode characters within the \texttt{U+0900--U+097F} range while resolving broken nuktas, broken diacritics, and invalid halant connectors; (b) length ratio constraint---$0.5 \le |S_{\text{dialect}}| / |S_{\text{standard}}| \le 2.0$ to prevent hallucinated expansions or truncations; (c) non-identity constraint---$S_{\text{dialect}} \neq S_{\text{standard}}$ to exclude trivially identical pairs; and (d) minimum token count---both source and target must contain $\ge$ 3 whitespace-separated tokens.
