...
Despite these advances, systematic evaluation of state-of-the-art ASR models against sub-dialect variation within individual languages remains sparse. Most recently, Dhasmana et al. \cite{dhasmana2026dialect} investigated cross-lingual ASR transfer across 30 Devanagari-script Indic varieties using the VAANI dataset, demonstrating that pre-training models fine-tuned on standardized languages exhibit severe performance degradation on non-standard dialects (e.g., reaching 88.1\% WER on Malvani) due to high orthographic variability and systemic pre-training bias toward Hindi. These findings directly motivate our proposed two-stage architecture, which combines robust acoustic ASR baselines with specialized sequence-to-sequence text normalization to bridge the sub-dialect error gap.
