...
\node (input) [box] {{ \bfseries RESPIN Source Corpus}\\[3pt]{\footnotesize Audio Recordings \& Dialect Transcripts}};
\node (gen) [engine, right=3.6cm of input] {{ \bfseries Raw Parallel Pair Generation}\\[3pt]{\footnotesize Gemma-4 LLM Translation}};

% Row 2: Verification Engine
\node (tier1) [stage, below=1.4cm of gen] {{ \bfseries Tier 1: Rule-Based Filter}\\[3pt]{\footnotesize Script Integrity \& Length Ratio}};
\node (tier2) [stage, left=3.6cm of tier1] {{ \bfseries Tier 2: LLM Semantic Auditor}\\[3pt]{\footnotesize LLaMA-3.1-8B Verification Pass}};
...
\node[font= \sffamily\bfseries\color{teal!80!black}, anchor=south, yshift=2pt] at (verbox.north) {Multi-Tier Verification Engine};
\end{scope}

% Row 3: Seq2Seq Fine-Tuning
\node (seq2seq) [model, below=1.5cm of tier2] {{ \bfseries Seq2Seq Model Training}\\[3pt]{\footnotesize IndicBART \& mT5-Small Evaluation}};
...
\hline
\makecell[l]{\textbf{Dialect Name}} & \makecell[c]{\textbf{Original Clean}\\\textbf{Pairs}} & \makecell[c]{\textbf{Raw Synthetic}\\\textbf{Generated}} & \makecell[c]{\textbf{Clean Synthetic}\\\textbf{Verified}} & \makecell[c]{\textbf{Corrupted / Rejected}\\\textbf{Pairs (\%)}} & \makecell[c]{\textbf{Total Clean}\\\textbf{Expanded Pairs}} \\
\hline
...
(1) \textbf{Held-Out RESPIN Test Benchmark} (Section~\ref{subsec:heldout_findings}, Table~\ref{tab:respin_comprehensive}, Figure~\ref{fig:respin_comprehensive}): Evaluates true unseen generalization on the official held-out test dataset comprising 2,170 utterances (\texttt{IISc\_RESPIN\_test\_mr}) after training models on the complete dataset; and
(2) \textbf{Mean 5-Fold Cross-Validation} (Sections~\ref{subsec:cv_findings}--\ref{subsec:ablation_findings}, Tables~\ref{tab:results_original_single}--\ref{tab:verification_ablation}, Figures~\ref{fig:single_dialect_cv}--\ref{fig:verification_ablation}): Evaluates in-domain model stability and dialectwise synthetic augmentation gains across 5 stratified train/test folds.}
...
\hline
\makecell[l]{\textbf{Model Scope}} & \makecell[l]{\textbf{Training Data}} & \makecell[l]{\textbf{Dialect / Subset}} & \makecell[c]{\textbf{Utts}} & \multicolumn{2}{c:}{\textbf{IndicBART (244M)}} & \multicolumn{2}{c|}{\textbf{mT5-Small (300M)}} \\
\cline{5-8}
& & & & \makecell[c]{\textbf{BLEU ($\uparrow$)}} & \makecell[c]{\textbf{WER ($\downarrow$)}} & \makecell[c]{\textbf{BLEU ($\uparrow$)}} & \makecell[c]{\textbf{WER ($\downarrow$)}} \\
\hline
...
Table~\ref{tab:results_original_single} presents the performance matrix on original single-dialect benchmarks.
...
Table~\ref{tab:results_original_multidialect} explains the dialect-wise breakdown of the multi-dialect model on original data.
...
Table~\ref{tab:results_expanded_single} presents the performance matrix on synthetically expanded single-dialect benchmarks.
...
Table~\ref{tab:results_expanded_multidialect} details the dialectwise evaluation breakdown of the multi-dialect model on expanded data.
