...
Rapid development of deep learning architectures and self-attention mechanisms \cite{vaswani2017attention} has shown significant progress across natural language understanding and generation tasks.
Availability of Large Language Models (LLMs) \cite{dubey2024llama3} and open-weight foundational architectures \cite{gemma2024} has allowed processing for text across domains such as finance, healthcare, and digital governance.
However, current systems remain inclined toward standardized, high-resource languages, showcasing dull performance when applied to non-standard regional dialects \cite{joshi2020state}.
Community surveys \cite{blaschke2024what,joshi2025survey} tend to prove that native dialect speakers face issues with language processing systems such as Automatic Speech Recognition and Machine Translation due to vocabulary and phonetic variations \cite{koenecke2020racial,kuparinen2023dialect}.


Dialect normalization is the task of transducing regional dialectal text into standard written language while keeping the underlying semantic intent unchanged \cite{kuparinen2023dialect}. 
It serves as an important layer for digital linguistic inclusion.
In countries like India, which has 22 officially recognized languages alongside hundreds of regional dialects spoken by 1.4 billion people, sub-regional varieties remain drastically neglected.
Marathi, an Indo-Aryan language spoken by over 83 million people in western India \cite{dabre2022indicbart}, is an excellent example of such disparity.
While research around Standard Marathi (language centered around Pune and its neighboring areas) has gained momentum , regional dialects remain underexplored.
%  exhibit marked phonological, lexical, and morpho-syntactic divergence from the normative written standard \cite{kulkarni2023variation,tarfe2024linguistic}.

Building normalization pipelines for regional Indic dialects primarily comes down to two core challenges:
(1) \textit{Extreme Data Scarcity}:Datasets mapping unstandardized regional dialects to normative standard forms are technically non-existent; and
(2) \textit{Morphological Complexity}: Marathi features a rich morphology, where noun case markers, postpositional affixes, and verbal tense suffixes undergo complex transformations. \cite{ansary2024unicode,partanen2019dialect}.

Addressing these limitations, this work provides a multi-dialect Marathi text normalization pipeline utilizing synthetic data augmentation and sequence-to-sequence transformer fine-tuning.
Our core contributions are:
\begin{itemize}
    \item \textbf{Parallel Corpus Generation}: Based on the transcripts of RESPIN dataset \cite{kumar2025respin}, we construct a parallel corpus mapping three Marathi dialects (Southern Konkan, Northern Konkan, and Varhadi) to Standard Marathi across agricultural and banking domains.
    \item \textbf{Automated Synthetic Augmentation with Verification}: We design and implement a synthetic data expansion pipeline using Gemma-4 \cite{gemma2024} with a verification pipeline that flags and removes corrupted pairs, expanding the parallel corpus.
\end{itemize}
...
Text normalization has primarily addressed two forms of orthographic variations: archaic spelling variation in historical manuscripts and unconstrained noise in digital User-Generated Content (UGC).
Early works were based on Levenshtein alignment, dictionary lookup, and Character-level Statistical Machine Translation (CSMT) \cite{pettersson2014multilingual,scherrer2016normalising}. 
Other evaluations on European languages showed that subword tokenization via Byte-Pair Encoding (BPE) and sentence-level contextual encoding reduced ambiguity compared to character-level transducers \cite{tang2018evaluation,bollmann2019historical,bawden2022automatic}.
Methodological paradigms transitioned from candidate edit-operation rankers like MoNoise \cite{vandergoot2019monoise} to standardized multilingual benchmarks such as MultiLexNorm \cite{vandergoot2021multilexnorm}, which confirmed that pre-processing via lexical normalization directly improves downstream dependency parsing and POS tagging. Recent work demonstrates that fine-tuning pre-trained byte-level models (byT5) and subword text-to-text transformers outperforms traditional statistical edit models for word-level sequence transduction \cite{samuel2021ufal,xue2021mt5}.
...
\begin{enumerate}
    \item \textbf{Baseline Saturation and Precision Dilution}: Varhadi already possessed a high original baseline (72.75--79.57 BLEU). We estimate that LLM-generated synthetic pairs may introduce minor stylistic variations or over-generalized suffix rules, slightly diluting exact N-gram matching against human reference texts.
...
The raw unverified dataset contains 19,914 parallel pairs, including 3,742 rejected or flawed pairs exhibiting Hindi language leakage, retained dialect residue, and garbled translations prior to multi-tier verification.
