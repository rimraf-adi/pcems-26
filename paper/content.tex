...
Initial works in European languages ustilised CSMT and statistical phrase-based models on Swiss German \cite{scherrer2016swiss}. 
They were eventually surpassed by neural architectures and pre-trained language models. Similar approaches have been developed for Finnish, Swedish, and Estonian dialects \cite{partanen2019dialect,hamalainen2020normalizing,hamalainen2022generating}. 
A multi-lingual study between four language families by Kuparinen et al. \cite{kuparinen2023dialect} suggested that fine-tuning sequence-to-sequence transformers outperformed rule-based and SMT baselines.
Beyond Europe, dialect processing has addressed structural divergence across East Asian and African-Asian varieties. Japanese regional dialects have been processed using phrase-level LSTM transducers \cite{abe2018multi}, while studies based on Mandarin scripts relied on acoustic-phonetic mapping \cite{zhao2022mandarin}. 
In Arabic NLP, the Conventional Orthography for Dialectal Arabic (CODA) framework \cite{habash2012coda} gave guidelines for Modern Standard Arabic (MSA), leading to multi-city dialect translation benchmarks (MADAR \cite{bouamor2018madar}, NADI \cite{abdulmageed2023nadi}) and morphologically-guided LLM prompting \cite{eryani2020arabic,dimakis2025dialect}.

\subsection{\textcolor{black}{Indic Dialect Landscape and Marathi NLP}}

Despite such linguistic diversity with over 1.4 billion speakers, Indic regional dialects remain severely underrepresented in language technology \cite{joshi2020state,zampieri2020nlp,joshi2025survey}. While pre-trained multilingual sequence-to-sequence models such as IndicBART \cite{dabre2022indicbart} and mT5 \cite{xue2021mt5} cover major scheduled languages via unified Devanagari scripts or mC4 corpora, they lack domain-specific fine-tuning on regional sub-dialects. Monolingual initiatives like L3Cube-MahaNLP \cite{joshi2022l3cube} offer Marathi encoder representations for classification, but do not support generative text standardization.
