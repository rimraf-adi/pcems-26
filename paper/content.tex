...
\begin{itemize}
    ...
    \item \textcolor{blue}{\textbf{Cross-Dialect \& Multilingual Model Benchmarks}: We provide a comprehensive empirical evaluation of existing multilingual models (mT5-small vs. IndicBART), demonstrating that evaluation results vary significantly across individual dialects (e.g., up to 81.00 BLEU on Varhadi) and establishing benchmark standards for Indic dialect normalization.}
\end{itemize}
...
\begin{tikzpicture}[
    ...
    model/.style={draw=blue!60!purple!75!black, fill=blue!60!purple!6, rounded corners=6pt, line width=1pt, align=center, text width=5.2cm, inner sep=7pt},
...
\hline
\multirow{8}{*}{Multi-Dialect} & \multirow{4}{*}{Original} & Southern Konkan & 559 & 26.08 & 52.98\% & \textbf{40.94} & \textbf{35.34\%} \\
...
\cline{2-8}
 & \multirow{4}{*}{Synthetically Expanded} & Southern Konkan & 559 & \textbf{52.15} & 36.50\% & 43.88 & \textbf{34.96\%} \\
...
\cline{2-7}
\makecell[l]{\small\textbf{Evaluated Subset}} & \makecell[c]{\small\textbf{Original}} & \makecell[c]{\small\textbf{Expanded}} & $\boldsymbol{\Delta}$ & \makecell[c]{\small\textbf{Original}} & \makecell[c]{\small\textbf{Expanded}} & $\boldsymbol{\Delta}$ \\
\hline
...
\caption{Impact of LLM synthetic data expansion ($\Delta$ BLEU score comparing synthetically expanded vs.\ original benchmarks on the held-out test set).}
...
The raw unverified dataset contains 19,914 parallel pairs, that included 3,751 rejected pairs that contained garbage translations.
...
\begin{enumerate}
    ...
    \item \textbf{mT5-Small Resilience}: While google/mT5-small showed resilience due to its 250,000-token multilingual capacity, training on unverified data still suffers a 8.30 BLEU point drop ,falling from 69.51 to 61.21 and a 4.40 chrF++ drop.
...
Individual dialect performance reached 81.00 BLEU and 91.21 chrF++ for Varhadi, demonstrating that compact pre-trained sequence-to-sequence transformers can achieve high-accuracy dialect normalization.
